...
def get_args():
    ...
    parser.add_argument('--result-file', type=str, default='results/CoIN/MiniGPTv2/Grounding/Finetune/merge.jsonl')
...
def eval_single(test_file, result_file):
    ...
    annotations = {str(grounding_test['question_id']): grounding_test for grounding_test in annotations}
    ...
    for result in results:
        ...
        bbox_string = grounding_gt['origin_answer_bbox']
        ...
        try:
            ...
            res = pattern.findall(pred_bbox)

            bbox_pred = [float(x) for x in res[:4]]
            if len(bbox_pred) != 4:
                continue
        except:
            continue
