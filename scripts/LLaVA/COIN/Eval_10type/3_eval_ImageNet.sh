...
RESULT_DIR="./results/CoIN/LLaVA-10Typev2/ImageNet"
...
for IDX in $(seq 0 $((CHUNKS-1))); do
    CUDA_VISIBLE_DEVICES=${GPULIST[$IDX]} python -m ETrain.Eval.LLaVA.CoIN.model_vqa \
        ...
        --question-file ./playground/Instructions_10type_slim/ImageNet/test.json \
...
python -m ETrain.Eval.LLaVA.CoIN.eval_ImagetNet \
    --test-file ./playground/Instructions_10type_slim/ImageNet/test.json \
    --result-file $output_file \
    --output-dir $RESULT_DIR/$STAGE \

python -m ETrain.Eval.LLaVA.CoIN.create_prompt \
    --rule ./ETrain/Eval/LLaVA/CoIN/rule.json \
    --questions ./playground/Instructions_10type_slim/ImageNet/test.json \
    --results $output_file \