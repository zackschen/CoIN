...
if [ ! -n "$2" ] ;then
    MODELPATH='./checkpoints/Instruction/Only_Pretrain_1.5_new/GQA/llava-1.5-7b-lora'
else
    MODELPATH=$2
fi

RESULT_DIR="./results/CLIT_new/GQA"
...
for IDX in $(seq 0 $((CHUNKS-1))); do
    CUDA_VISIBLE_DEVICES=${GPULIST[$IDX]} python -m llava.eval.Instruction_CC.model_gqa \
        ...
        --question-file ./playground/Instructions/GQA/test_new.json \
...
python playground/create_prompt.py \
    ...
    --questions ./playground/Instructions/GQA/test_new.json \
    --results $output_file \