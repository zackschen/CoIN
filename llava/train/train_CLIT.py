...
class DataArguments:
    ...
    dataset_json_path: str = field(default=None,
                           metadata={"help": "Path to the datasets config data."})
...
def train():
    ...
    if data_args.dataset_json_path is not None:
        dataset_json = json.load(open(data_args.dataset_json_path, "r"))
        for dataset_name, dataset in dataset_json.items():
            rank0_print('Trainning on the {}'.format(dataset_name))
            data_path = dataset["data_path"]
            output_dir = dataset["output_dir"]
            per_device_train_batch_size = dataset["per_device_train_batch_size"]
